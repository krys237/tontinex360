...
import type {
  User, TokenPair, LoginRequest, RegisterRequest, Association,
} from '@/lib/types/auth';
...
export const authApi = {
  ...
  // Associations (montées sur /auth/ ET /associations/ — on garde /associations/)
  myAssociations: () =>
    api.get<{ associations: Association[]; active_slug?: string | null }>(
      '/associations/associations/'
    ).then(r => r.data),

  selectAssociation: (slug: string) =>
    api.post<{ active_association: Association }>(
      '/associations/associations/select/', { slug }
    ).then(r => r.data),

  createAssociation: (data: {
    name: string; slug: string;
    description?: string; city?: string; region?: string;
  }) =>
    api.post<{
    ...
    }>('/associations/associations/create/', data).then(r => r.data),
  ...
  updateAssociation: (slug: string, data: Partial<Association> | FormData) => {
    ...
    return api.patch<Association>(
      `/associations/associations/${slug}/`,
      data,
